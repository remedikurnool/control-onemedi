-- Enhanced Location Management System for Multi-Service Healthcare Platform
...
CREATE TABLE public.location_service_zones (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  location_id UUID REFERENCES public.locations(id) ON DELETE CASCADE,
  service_type public.service_type NOT NULL,
...
CREATE TABLE public.pincode_service_mapping (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_service_locations_location_service ON public.service_locations(location_id, service_type);
CREATE INDEX idx_service_centers_location_type ON public.service_centers(location_id, service_type);
CREATE INDEX idx_service_centers_pincode ON public.service_centers(pincode);
CREATE INDEX idx_location_zones_service_type ON public.location_service_zones(service_type);
CREATE INDEX idx_location_analytics_date ON public.location_analytics(date);
CREATE INDEX idx_pincode_mapping_pincode ON public.pincode_service_mapping(pincode);
CREATE INDEX idx_pincode_mapping_service_type ON public.pincode_service_mapping(service_type);

-- Insert default business configuration for Kurnool-based operations
INSERT INTO public.business_configuration (config_key, config_value, description) VALUES 
